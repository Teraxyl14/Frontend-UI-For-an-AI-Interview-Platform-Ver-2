...
const InterviewScreen: React.FC<ScreenProps> = ({ navigate, interviewMode, drillType, aiState, setAiState }) => {
    ...
    const handleDoneAnswering = () => {
        ...
        if (currentQuestionIndex < questionFlow.length - 1) {
            ...
            // Transition to the "asking questions" phase
            setAiState?.('thinking');
            setTimeout(() => {
    ...
    const renderAnsweringPhase = () => (
        <>
            <div className="h-14 flex items-center justify-center">
                ...
                {aiState === 'listening' && (
                    <button onClick={handleDoneAnswering} className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-8 rounded-full text-lg animate-fade-in">
                        Done Answering
                    </button>
                )}
                {aiState === 'thinking' && (
                    <button className="bg-purple-600/50 text-white font-bold py-3 px-12 rounded-full text-lg cursor-wait flex items-center gap-3" disabled>
            ...
            <div className="interview-control-bar p-2 flex items-center gap-2">
                ...
                <button title="End Session" onClick={handleDoneAnswering} className="interview-control-btn end-call"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg></button>
            </div>
        </>
    );
    ...
    return (
        <div className="w-full h-full flex flex-col items-center justify-between">
            ...
            <DraggableVideo transcript={transcript} />
